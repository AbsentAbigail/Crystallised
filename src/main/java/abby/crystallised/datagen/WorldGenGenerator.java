...
public class WorldGenGenerator implements DataProvider {
    ...
    public void generate(WorldGenExporter exporter) {
        GemType.forEach((name, type) -> {
            ...
            exporter.accept(
                    id,
                    generateConfiguredFeatureJson(id, deepslateId, 6),
                    generatePlacedFeatureJson(id, 28, 320, -64)
            );
        });
    }
    ...
    private JsonObject generatePlacedFeatureJson(Identifier identifier, int amount, int maxHeight, int minHeight) {
        JsonObject json = new JsonObject();
        ...
        JsonObject heightRange = getHeightRange(maxHeight, minHeight);
    ...
    @NotNull
    private static JsonObject getHeightRange(int maxHeight, int minHeight) {
